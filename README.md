...
> 请按照项目里的 AGENTS.md 执行完整图书翻译流程，翻译 `input/我的书.pdf`。请依次完成 OCR、目录识别、章节整理、OCR 文字润色、术语统一、翻译、本地校验和 EPUB 打包。润色及其校验完成前不要提取术语或翻译。所有结果都要写入项目文件，不要只把译文发在聊天窗口里。遇到必须由我决定的事情再询问我；完成并通过校验后告诉我最终 EPUB 文件在哪里。
