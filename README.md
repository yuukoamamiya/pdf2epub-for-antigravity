...
> 请先阅读项目里的 AGENTS.md 和 README.md。请检查这个项目是否已经具备运行条件，并帮我完成首次配置。我要用它翻译图书，请根据本机可用情况配置 OCR；优先使用安全、可用的本地或已配置服务。如果需要我提供 OCR 服务地址、账号或密钥，请先告诉我应该提供什么，不要把密钥写进项目文件或提交到 GitHub。除非我明确要求，不要修改或删除原始图书。

AI 会检查环境并创建或调整需要的本地配置。你不需要自己填写复杂配置。
