...
> 请先阅读项目里的 AGENTS.md 和 README.md。请检查这个项目是否已经具备运行条件，并帮我完成首次配置。我要用它翻译图书，请优先检查并配置本地 PaddleOCR；先确认当前项目版本是否真正支持 PaddleOCR，如果支持就帮我安装和配置，如果不支持就明确告诉我，不要写入一个程序无法识别的配置。只有本地 OCR 不可用时，再考虑远端 OCR，并先告诉我需要什么服务地址、账号或密钥。不要把密钥写进项目文件或提交到 GitHub。除非我明确要求，不要修改或删除原始图书。

AI 会检查环境并创建或调整需要的本地配置。PaddleOCR 是推荐的本地 OCR 方案；远端 OCR 只是备用方案。你不需要自己填写复杂配置。
