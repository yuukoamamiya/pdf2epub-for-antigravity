...
> 请先阅读项目里的 AGENTS.md 和 README.md。请检查这个项目是否已经具备运行条件，并帮我完成首次配置。我要用它翻译图书：如果项目里已经有可用的远端 OCR 配置，请保留并继续使用，不要擅自改成本地 OCR；如果没有远端 OCR，再询问我是否要尝试本地 PaddleOCR，并先确认当前项目版本是否真正支持。不要写入一个程序无法识别的配置。若需要我提供服务地址、账号或密钥，请先告诉我应该提供什么。不要把密钥写进项目文件或提交到 GitHub。除非我明确要求，不要修改或删除原始图书。

AI 会检查环境并保留或创建需要的配置。已经有远端 OCR 的用户继续使用远端；没有远端服务的用户，可以让 AI 检查是否能配置本地 PaddleOCR。你不需要自己填写复杂配置。
