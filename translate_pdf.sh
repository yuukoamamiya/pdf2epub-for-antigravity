...
# 自动定位 Chandra Access 凭证
if [ -z "$CHANDRA_ACCESS_CREDENTIALS" ] && [ -f "$HOME/.config/pdf2epub/chandra-access.json" ]; then
    export CHANDRA_ACCESS_CREDENTIALS="$HOME/.config/pdf2epub/chandra-access.json"
fi
...
" 2>&1 | tee -a "$LOG_DIR/refine.log"
    ...
    if [ $TOC_EXIT -ne 0 ]; then
        ...
        echo "║   4. 重新运行: bash translate_pdf.sh       ║"
        echo "╚═══════════════════════════════════════════╝"
        exit 1
    fi
    echo "✅ TOC 检测通过"
...
$PDF2EPUB_CMD -c "$CONFIG" translate --resume 2>&1 | tee "$LOG_DIR/translate.log"

echo "=== Step 5: Build EPUB ==="
OUT_DIR="output/${BOOK_TITLE}"

# 中文版
$PDF2EPUB_CMD -c "$CONFIG" build-epub --translated 2>&1 | tee "$LOG_DIR/build_cn.log"
CN_FILE=$(ls -t "$OUT_DIR"/*.epub 2>/dev/null | head -1 || true)
CN_BASENAME=$(basename "$CN_FILE" 2>/dev/null || true)

# 原版
$PDF2EPUB_CMD -c "$CONFIG" build-epub 2>&1 | tee "$LOG_DIR/build_raw.log"
RAW_FILE=$(ls -t "$OUT_DIR"/*.epub 2>/dev/null | head -1 || true)
RAW_BASENAME=$(basename "$RAW_FILE" 2>/dev/null || true)

echo "---"
if [ "$CN_BASENAME" = "$RAW_BASENAME" ] && [ -n "$CN_BASENAME" ]; then
    # 同名 → 原版覆盖了中文版，清场后分别重建并加后缀
    rm -f "$OUT_DIR"/*.epub

    $PDF2EPUB_CMD -c "$CONFIG" build-epub --translated 2>&1 | tee -a "$LOG_DIR/build_cn.log"
    $PYTHON_CMD -c "
import shutil
from pathlib import Path
out = Path(r'''$OUT_DIR''')
epubs = sorted([p for p in out.glob('*.epub') if '（' not in p.name and '(' not in p.name], key=lambda p: p.stat().st_mtime, reverse=True)
if epubs:
    target = out / r'''${BOOK_TITLE}（中文版）.epub'''
    shutil.move(str(epubs[0]), str(target))
"
    echo "📗 中文版: $OUT_DIR/${BOOK_TITLE}（中文版）.epub"

    $PDF2EPUB_CMD -c "$CONFIG" build-epub 2>&1 | tee -a "$LOG_DIR/build_raw.log"
    $PYTHON_CMD -c "
import shutil
from pathlib import Path
out = Path(r'''$OUT_DIR''')
epubs = sorted([p for p in out.glob('*.epub') if '（' not in p.name and '(' not in p.name], key=lambda p: p.stat().st_mtime, reverse=True)
if epubs:
    target = out / r'''${BOOK_TITLE}（原版）.epub'''
    shutil.move(str(epubs[0]), str(target))
"
    echo "📕 原版: $OUT_DIR/${BOOK_TITLE}（原版）.epub"
else
    echo "📗 中文版: $CN_FILE"
    echo "📕 原版: $RAW_FILE"
fi
